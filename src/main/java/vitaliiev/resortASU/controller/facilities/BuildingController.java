...
import vitaliiev.resortASU.service.facilities.BuildingService;
...
public class BuildingController {
    ...
    public String list(Model model) {
        if (!model.containsAttribute("entities")) {
            List<Building> entities = service.findAll();
    ...
    public String find(@ModelAttribute(name = "newEntity") Building entity,
                       RedirectAttributes redirectAttributes) {
        List<Building> entities = service.find(entity);
        redirectAttributes.addFlashAttribute("entities", entities);
        return "redirect:" + REQUEST_MAPPING;
    }

    @PostMapping(REQUEST_MAPPING + "/create")
    public String create(@ModelAttribute(name = "newEntity") Building entity, RedirectAttributes redirectAttributes) {
        try {
    ...
    public String read(@PathVariable Integer id, Model model) {
        Building entity = service.findById(id);
        if (entity != null) {
            model.addAttribute("fragment", FRAGMENT_NAME);
